...
| **Drag the price axis (Y)** (MBO mode) | Zoom the **price range** — drag **down = zoom in** (taller bars), **up = zoom out**. Adjusts `heatYTicks` directly (LWC's own axis scaling is turned off in MBO so it can't snap back), so the zoom **holds on release**; price stays auto-centered. Also: **wheel on the axis** / **Shift+wheel**. |
